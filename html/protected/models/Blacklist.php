...
class Blacklist extends CActiveRecord
{
	...
	public static function lookup($acc = '', $txid = '') {

		if($acc) {

			$query = Yii::app()->db->createCommand();
			$query->select('*');
			$query->from('tbl_blacklist');
			$query->where("type = 'acc' AND value = :acc", array(':acc'=>$acc));
			$query->limit(1);
		...
		if($txid) {
			...
			$query->select('*');
			$query->from('tbl_blacklist');
			$query->where("type = 'txid' AND value = :txid", array(':txid'=>$txid));
			$query->limit(1);
			$result = $query->queryAll();
			if(!empty($result)) {
				return true;
